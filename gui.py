...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowTitle("Real-Time LAN Packet Sniffer and Protocol Security Inspection System")
        ...
        self.traffic_label = QLabel("Traffic: 0 B")
        self.bandwidth_label = QLabel("Bandwidth: 0 B/s")
        self.alert_count_label = QLabel("Security Alerts: 0")
        ...
        protocol_group = QGroupBox("Protocol Statistics")
        ...
        for protocol in self.protocol_counts:
            ...
            self.protocol_labels[protocol] = label
        ...
        table_group = QGroupBox("Live Packet Capture")
    ...
    def receive_packet(self, data):
        ...
        """
        This function will receive packet dictionaries
        from Ananya's backend.
...

        {
            ...
            "protocol": "HTTP",
            "packet_length": 100,
            "security_warning": "Unencrypted HTTP traffic detected"
        }
        """

        self.packet_count += 1

        packet_length = data.get("packet_length") or 0
        ...
        security_warning = data.get("security_warning")
        ...
        if self.capture_start_time:
            ...
            if elapsed > 0:
                ...
                self.bandwidth_label.setText(
                    f"Bandwidth: {self.format_bytes(bandwidth)}/s"
                )

        self.alert_count_label.setText(
            f"Security Alerts: {self.security_alert_count}"
        )

        # Update protocol counters

        for protocol_name, label in self.protocol_labels.items():
        ...
        values = [
            ...
            str(data.get("source_port") or "-"),
            str(data.get("destination_port") or "-"),
            str(packet_length),
            security_warning or "-",
        ]
        ...
        # Store complete packet dictionary with the row

        self.packet_table.item(row, 0).setData(
            Qt.UserRole,
            data
        )
